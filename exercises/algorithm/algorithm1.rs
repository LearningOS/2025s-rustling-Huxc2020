...
impl<T: PartialOrd + Clone> LinkedList<T> {
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        ...
        while let (Some(ptr_a), Some(ptr_b)) = (curr_a, curr_b) {
            unsafe {
                if (*ptr_a.as_ptr()).val <= (*ptr_b.as_ptr()).val {
                    result.add((*ptr_a.as_ptr()).val.clone());
                    curr_a = (*ptr_a.as_ptr()).next;
                } else {
                    result.add((*ptr_b.as_ptr()).val.clone());
        ...
        // 处理剩余的节点
        while let Some(ptr) = curr_a {
            unsafe {
                result.add((*ptr.as_ptr()).val.clone());
                curr_a = (*ptr.as_ptr()).next;
            }
        }
        while let Some(ptr) = curr_b {
            unsafe {
                result.add((*ptr.as_ptr()).val.clone());
                curr_b = (*ptr.as_ptr()).next;
            }
        }
